        ...
        if any([STOP_DUPLICATE, TORRENT_DIRECT_LIMIT, ZIP_UNZIP_LIMIT, STORAGE_THRESHOLD]):
            ...
            if STOP_DUPLICATE:
                ...
                if sname is not None:
                    ...
                    if smsg:
                        dl.getListener().onDownloadError('⚡️Cool Bro Someone Already Generate This !\n\n')
                        api.remove([download], force=True, files=True)
                        return sendMarkup("File", dl.getListener().bot, dl.getListener().message, button)
...
def __onDownloadStopped(api, gid):
    ...
    dl = getDownloadByGid(gid)
    if dl:
        dl.getListener().onDownloadError('⚡️ DUDE THIS IS ZOMBIE TORRENT MEANS DEAD')
