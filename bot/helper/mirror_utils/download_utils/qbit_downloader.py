...
class QbDownloader:
                ...
                if len(tor_info) == 0:
                    while True:
                        ...
                        if len(tor_info) > 0:
                            ...
                            msg = "⚡️I Think This Torrent File Is Sucks Check By YourSelf And Send Me Back Bro If Again It's Not Work Go To Bed And Sleep !!"
            ...
            if tor_info.state == "metaDL":
                ...
                if not self.__dupChecked and STOP_DUPLICATE and ospath.isdir(f'{self.__path}') and not self.__listener.isLeech:
                    ...
                    if qbname is not None:
                        ...
                        if qbmsg:
                            self.__onDownloadError("⚡️Cool Bro Someone Already Generate This !")
                            sendMarkup("File", self.__listener.bot, self.__listener.message, button)
                ...
                if not self.__rechecked and 0.99989999999999999 < tor_info.progress < 1:
                    ...
                    self.__onDownloadError("⚡️ DUDE THIS IS ZOMBIE TORRENT MEANS DEAD")
    ...
    def cancel_download(self):
        if self.is_seeding:
            ...
            self.__onDownloadError('⚡️Bro Your Processing Is Stop Man !')
...
def get_confirm(update, context):
    ...
    if not qbdl:
        query.answer(text="⚡️Bro task has been cancelled!", show_alert=True)
        query.message.delete()
    elif user_id != qbdl.listener().message.from_user.id:
        query.answer(text="⚡️ Man What You Doing Task Is Not For You Bro, STFU !", show_alert=True)
    elif data[1] == "pin":
