...
def setLeechType(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer(text="Cool DUDE Your File Will Deliver As Document", show_alert=True)
        ...
        query.answer(text="Cool DUDE Your File Will Deliver As Media", show_alert=True)
        editLeechType(message, query)
    elif data[2] == "thumb":
        ...
        if ospath.lexists(path):
            ...
            query.answer(text="Goodboy Thumbnail Removed!", show_alert=True)
            editLeechType(message, query)
        else:
...
def setThumb(update, context):
    ...
    if reply_to is not None and reply_to.photo:
        ...
        msg = f"DUDE Your Favorite thumbnail Saved {update.message.from_user.mention_html(update.message.from_user.first_name)}."
        sendMessage(msg, context.bot, update.message)
    else:
        sendMessage("Reply With Your Favorite Thumbnail To Set Your Thumbnail Man !.", context.bot, update.message)
