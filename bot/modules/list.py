...
def list_buttons(update, context):
    ...
    sendMarkup('Choose Options.', context.bot, update.message, button)
...
def select_type(update, context):
    ...
    if user_id != int(data[1]):
        ...
        query.answer()
        return editMessage("GoodBoy List has been canceled", msg)
    query.answer()
    item_type = data[2]
    editMessage(f"<b>Searching  <i>{key}</i></b>", msg)
...
def _list_drive(key, bmsg, item_type):
    ...
    if button:
        ...
        editMessage(f'Oops No Results<i>{key}</i>', bmsg)
