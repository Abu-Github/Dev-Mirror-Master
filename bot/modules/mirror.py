...
class MirrorListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name: str):
        ...
        if self.isLeech:
            ...
            msg += f'\n\n<b> </b>{self.tag} <b> </b>'
            msg += f'\n<b> </b> {get_readable_time(time() - self.message.date.timestamp())}'
            ...
            if INDEX_URL is not None:
                ...
                if ospath.isdir(f'{DOWNLOAD_DIR}/{self.uid}/{name}'):
                    ...
                    if VIEW_LINK:
                        ...
                        buttons.buildbutton("Team Cloud", share_urls)
...
def _mirror(bot, message, isZip=False, extract=False, isQbit=False, isLeech=False, pswd=None, multi=0, qbsd=False):
    ...
    if FSUB:
        try:
            ...
            if user.status not in ['member', 'creator', 'administrator']:
                buttons.buildbutton("𝗥𝗲𝘃𝗼𝗸𝗲 𝗟⚡️𝗻𝗸𝘀", f"https://t.me/{CHANNEL_USERNAME}")
                reply_markup = InlineKeyboardMarkup(buttons.build_menu(1))
                return sendMarkup(f"<b>⚡️DUDE {uname}️,\nYou Are Not Subscribed To 𝗗𝗨𝗠𝗕 - 𝗥𝗲𝘃𝗼𝗸𝗲 𝗟⚡️𝗻𝗸𝘀 Yet.\nJoin us <u>Use Bots Feel Freely.</u></b>", bot, message, reply_markup)
        except Exception as e:
            LOGGER.info(str(e))
    ...
    if BOT_PM and message.chat.type != 'private':
        try:
            ...
            startwarn = f"Dear {uname},\n\n<b>Dude you haven't started me in PM (Private) yet.</b>\n\n" \
                        f" "
